...
function FileKindIcon({ item }: { item: FileUploadItem }) {
  const extension = item.name.includes(".")
  ...
  if (type.startsWith("image/")) return <FileImage className={iconClassName} />;
  if (type.startsWith("video/")) return <FileVideo className={iconClassName} />;
  if (type.startsWith("audio/")) return <FileAudio className={iconClassName} />;
  if (
    type.includes("zip") ||
...
function FileUploadRow({
  ...
  return (
    <motion.li
      ...
      className={cn(
        "relative overflow-hidden rounded-none border border-border bg-background p-3",
        classNames?.item,
      )}
    >
      <div className="flex items-center gap-3">
        <div
          className={cn(
            "grid h-11 w-11 shrink-0 place-items-center rounded-none bg-muted text-muted-foreground",
            classNames?.leading,
          )}
        >
        ...
        <div className={cn("min-w-0 flex-1", classNames?.content)}>
            ...
            <div className="flex shrink-0 items-center gap-1">
              ...
              {status === "error" ? (
                <button
                  ...
                  className={cn(
                    "grid h-7 w-7 place-items-center rounded-none text-muted-foreground transition-colors duration-150 hover:bg-muted hover:text-foreground active:scale-95",
                    classNames?.action,
                  )}
                >
          ...
          {showProgress ? (
            <div
              ...
              className={cn(
                "mt-3 h-1.5 overflow-hidden rounded-none bg-muted",
                classNames?.progress,
              )}
            >
              <motion.div
  ...
  return (
    <div className={cn("flex w-full flex-col gap-3", className, classNames?.root)}>
      ...
      <button
        ...
        className={cn(
          "group relative flex w-full overflow-hidden rounded-none border border-dashed border-border bg-background outline-none",
      ...
      >
        <motion.span
          ...
          className={cn(
            ...
            centered
              ? "h-16 w-16 rounded-none border border-border"
